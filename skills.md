...
For multi-file replacements, combine with `find`:
```
# Not directly composable through shell_exec (no shell interpreter).
# Call once per file found via shell_exec find, then loop in your logic.
```
